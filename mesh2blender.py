...
class mesh2scene(bpy.types.Operator):
    ...
    bl_description = 'Import mesh to Blender. If one needs to run MMC photon simulations, please remember to set the ' \
                     'optical properties to each region '
    ...
    def importmesh(self):
        ...
        if (not np.any(np.isinf(bbx['min']))) and (not np.any(np.isinf(bbx['max']))):
            light_object.location = ((bbx['min'][0] + bbx['max'][0]) * 0.5, (bbx['min'][1] + bbx['max'][1]) * 0.5,
